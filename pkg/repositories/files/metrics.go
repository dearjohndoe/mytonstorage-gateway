...
func (m *metricsMiddleware) HasBan(ctx context.Context, bagID string) (ok bool, err error) {
	...
	defer func(s time.Time) {
		labels := []string{"HasBan", strconv.FormatBool(err != nil)}
...
func (m *metricsMiddleware) GetReports(ctx context.Context, limit int, offset int) (reports []db.Report, err error) {
	m.inFlightGauge.Inc()
	defer func(s time.Time) {
		labels := []string{"GetReports", strconv.FormatBool(err != nil)}
		m.reqCount.WithLabelValues(labels...).Inc()
		m.reqDuration.WithLabelValues(labels...).Observe(time.Since(s).Seconds())
		m.inFlightGauge.Dec()
	}(time.Now())
...
func (m *metricsMiddleware) GetReportsByBagID(ctx context.Context, bagID string) (reports []db.Report, err error) {
	m.inFlightGauge.Inc()
	defer func(s time.Time) {
		labels := []string{"GetReportsByBagID", strconv.FormatBool(err != nil)}
		m.reqCount.WithLabelValues(labels...).Inc()
		m.reqDuration.WithLabelValues(labels...).Observe(time.Since(s).Seconds())
		m.inFlightGauge.Dec()
	}(time.Now())
...
func (m *metricsMiddleware) GetBan(ctx context.Context, bagID string) (status *db.BanStatus, err error) {
	m.inFlightGauge.Inc()
	defer func(s time.Time) {
		labels := []string{"GetBan", strconv.FormatBool(err != nil)}
		m.reqCount.WithLabelValues(labels...).Inc()
		m.reqDuration.WithLabelValues(labels...).Observe(time.Since(s).Seconds())
		m.inFlightGauge.Dec()
	}(time.Now())
...
func (m *metricsMiddleware) GetAllBans(ctx context.Context, limit int, offset int) (bans []db.BanStatus, err error) {
	m.inFlightGauge.Inc()
	defer func(s time.Time) {
		labels := []string{"GetAllBans", strconv.FormatBool(err != nil)}
		m.reqCount.WithLabelValues(labels...).Inc()
		m.reqDuration.WithLabelValues(labels...).Observe(time.Since(s).Seconds())
		m.inFlightGauge.Dec()
	}(time.Now())
...
func (m *metricsMiddleware) AddReport(ctx context.Context, report db.Report) (err error) {
	m.inFlightGauge.Inc()
	defer func(s time.Time) {
		labels := []string{"AddReport", strconv.FormatBool(err != nil)}
		m.reqCount.WithLabelValues(labels...).Inc()
		m.reqDuration.WithLabelValues(labels...).Observe(time.Since(s).Seconds())
		m.inFlightGauge.Dec()
	}(time.Now())
...
func (m *metricsMiddleware) UpdateBanStatus(ctx context.Context, statuses []db.BanStatus) (err error) {
	m.inFlightGauge.Inc()
	defer func(s time.Time) {
		labels := []string{"UpdateBanStatus", strconv.FormatBool(err != nil)}
		m.reqCount.WithLabelValues(labels...).Inc()
		m.reqDuration.WithLabelValues(labels...).Observe(time.Since(s).Seconds())
		m.inFlightGauge.Dec()
	}(time.Now())
...
func NewMetrics(reqCount *prometheus.CounterVec, reqDuration *prometheus.HistogramVec, inFlight prometheus.Gauge, repo Repository) Repository {
